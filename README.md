...
and `/downloads` lists `artifacts/<session>/downloads` (web_fetch PDFs). Image
...
The default-model row reuses the filtered model catalog. Selecting a model
persists `[models].default` and switches the active session; selecting
...
When `flash` or `hold` is active, a second click on assistant text shows
`Want double-click to select? /settings → Text selection · Ctrl+Y: enable now`
for up to 20 visible seconds, at most three times per session. The timer pauses
while another view covers the status row, prompt edits retire the hint, and
`Ctrl+Y` switches and persists `word_select` only while the unchanged hint is
visible. Set `word_select = false` under `[ui.contextual_hints]` to hide it.
...
`NO_COLOR`, Apple Terminal truecolor/OSC 52 limits, iTerm2 OSC 52 permission and VS Code-family SSH non-ASCII copy caveats, SSH/container OSC 52 delivery-unverified and delivery-unavailable clipboard gaps, Voice microphone detection (and a missing-input warning when capture is supported), VTE/xterm.js Shift+Enter newline-fallback gaps, local WezTerm Kitty-keyboard-off guidance, SSH WezTerm recovery via XTVERSION (TUI probe) with SSH-specific Shift+Enter notes; recorded self-report appears in doctor Facts/`--json` as `xtversion`, default notification BEL-fallback and focus-tracking gaps, and Byobu-on-screen — without calling the model.
...
`~/.tmux.conf` options; `colorterm` exports `COLORTERM=truecolor` in Bash/zsh/fish startup files. Preview without `--yes`.
...
refetch the catalog; server-pushed tool list changes include the refreshed
