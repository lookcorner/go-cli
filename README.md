...
`NO_COLOR`, Apple Terminal truecolor/OSC 52 limits, iTerm2 OSC 52 permission and VS Code-family SSH non-ASCII copy caveats, SSH/container OSC 52 delivery-unverified and delivery-unavailable clipboard gaps, Voice microphone detection (and a missing-input warning when capture is supported), VTE/xterm.js Shift+Enter newline-fallback gaps, local WezTerm Kitty-keyboard-off guidance, SSH WezTerm recovery via XTVERSION (TUI probe) with SSH-specific Shift+Enter notes, default notification BEL-fallback and focus-tracking gaps, and Byobu-on-screen — without calling the model.
