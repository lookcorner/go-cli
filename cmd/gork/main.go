...
func parseRunOptions(args []string, stderr io.Writer) (options, *flag.FlagSet, error) {
	...
	flags.StringVar(&opts.resume, "resume", "", "resume a session ID, title, path, or the current workspace's latest session")
	flags.StringVar(&opts.resume, "r", "", "resume a session ID, title, or path")
	flags.StringVar(&opts.resume, "load", "", "resume a session ID, title, or path")
...
func runOnce(args []string, stdin io.Reader, stdout, stderr io.Writer) error {
	...
	if len(args) > 0 && args[0] == "leader" {
		return runLeader(args[1:], stdout, stderr)
	}
	...
	registry.ConfigureBash(bashTimeout(cfg.Toolset.Bash.TimeoutSeconds), bashTimeout(cfg.Toolset.Bash.MaxTimeoutSeconds), bashOutputLimit(cfg.Toolset.Bash.OutputByteLimit), cfg.Toolset.Bash.CommandPrefix)
	...
	if opts.tui {
		...
		exit, err := tui.Run(ctx, runner, tuiBridge, prompt, opts.previousID, resumedTranscript, ws.Root(), cfg.Model, tui.UIOptions{
			...
			ContextualImage:      cfg.UI.ContextualHints.ImageInput,
			...
			CombineQueuedPrompts: cfg.UI.CombineQueuedPrompts,
			...
			VoiceKeybindEnabled: cfg.UI.VoiceKeybindEnabled,
			...
			SetCombineQueued:  func(enabled bool) error { return config.UpdateCombineQueuedPrompts(opts.configPath, enabled) },
			SetVoiceKeybind:   func(enabled bool) error { return config.UpdateVoiceKeybindEnabled(opts.configPath, enabled) },
			...
			SetContextualPlan: func(enabled bool) error {
				return config.UpdateContextualPlanModeHint(opts.configPath, enabled)
			},
			SetContextualImage: func(enabled bool) error {
				return config.UpdateContextualImageInputHint(opts.configPath, enabled)
			},
...
func runACP(cfg config.Config, opts options, allowRules, askRules, denyRules []string, tokenProvider api.TokenProvider, stdin io.Reader, stdout, stderr io.Writer) error {
	...
	server.Factory = func(
		...
		registry.ConfigureBash(bashTimeout(sessionCfg.Toolset.Bash.TimeoutSeconds), bashTimeout(sessionCfg.Toolset.Bash.MaxTimeoutSeconds), bashOutputLimit(sessionCfg.Toolset.Bash.OutputByteLimit), sessionCfg.Toolset.Bash.CommandPrefix)
...
func bashTimeout(seconds float64) time.Duration {
	...
	milliseconds := math.Round(min(seconds, 36000) * 1000)
	return time.Duration(max(milliseconds, 1)) * time.Millisecond
}
