...
func TestCompletionCandidatesFollowCommandTree(t *testing.T) {
	tests := []struct {
		...
		{nil, []string{"--always-approve", "--cwd", "--json-schema", "--max-turns", "--output-format", "--prompt-file", "--prompt-json", "--reasoning-effort", "--single", "-m", "-p", "-V", "completions", "dashboard", "models", "plugin", "share", "trace", "version", "wrap", "worktree"}},
