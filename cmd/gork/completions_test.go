...
func TestCompletionCandidatesFollowCommandTree(t *testing.T) {
	tests := []struct {
		...
		{[]string{"agent", "--"}, []string{"--always-approve", "--bind", "--leader", "--model", "--no-leader", "--reasoning-effort", "--secret"}},
		...
		{[]string{"plugin", "install", "--"}, []string{"--trust"}},
		...
		{[]string{"doctor", "fix", ""}, []string{"ssh-wrap", "tmux-clipboard", "dcs-passthrough", "tmux-extended-keys", "--yes"}},
		...
		{[]string{"plugin", "marketplace", "add", "--"}, []string{"--force"}},
