...
func TestCompletionCandidatesFollowCommandTree(t *testing.T) {
	tests := []struct {
		...
		{nil, []string{"--always-approve", "--continue", "--cwd", "--fork-session", "--json-schema", "--load", "--max-turns", "--output-format", "--prompt-file", "--prompt-json", "--reasoning-effort", "--session-id", "--single", "-c", "-m", "-p", "-r", "-s", "-V", "completions", "dashboard", "models", "plugin", "share", "trace", "version", "wrap", "worktree"}},
