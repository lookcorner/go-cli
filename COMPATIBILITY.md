...
| MCP | partial | Stdio and Streamable HTTP lifecycle, session/protocol headers, JSON/SSE responses, version negotiation, paginated tool/resource/prompt discovery, resource reads, prompt rendering, tool calls and shutdown work; standalone SSE transport, sampling, subscriptions and list-change reload remain |
| ACP | partial | Protocol v1 JSON-RPC stdio supports independent sessions, active-session list/filter metadata, prompt continuation, streamed text/tool lifecycle updates, cancellation, close, client-provided stdio MCP servers, embedded text context and tool-correlated permission requests; persistent load/resume, modes, image/audio prompts and ACP client mode remain |
