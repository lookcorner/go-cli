...
| MCP | partial | Stdio, Streamable HTTP and standalone SSE lifecycle, endpoint/origin validation, session/protocol headers, JSON/SSE responses, version negotiation, paginated tool/resource/prompt discovery, resource reads, prompt rendering, tool calls, tool list-change reload and shutdown work; sampling, resource subscriptions, ACP-provided HTTP/SSE servers and prompt/resource list reload remain |
| ACP | partial | Protocol v1 JSON-RPC stdio supports independent sessions, persistent list/filter metadata, load with completed text-history replay, resume without replay, prompt continuation, text/base64/remote-image prompt blocks, embedded text context, streamed text/tool lifecycle updates, cancellation, close, client-provided stdio MCP servers and tool-correlated permission requests; modes, non-text history replay, audio prompts and ACP client mode remain |
