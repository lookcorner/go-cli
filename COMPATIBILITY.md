...
| Workspace file tools | partial | Gork-compatible `read_file`, `list_dir`, `grep`, and `search_replace` schemas cover text/PPTX reads with negative offsets, Gitignore-aware bounded directory trees with large-subtree extension summaries, ripgrep filters, atomic create/overwrite, exact/all replacement, CRLF preservation and safe Unicode typography fallback; PDF and multimodal image reads remain |
