...
ACP session `_meta.mcpConfig.<server>.startupTimeoutMs` overrides that stack
for both regular and SDK-backed MCP servers.
...
MCP output note: `[mcp].max_output_bytes` defaults to 20,000 bytes and honors
requirements, environment, local, and remote precedence. Oversized text and
error results keep a UTF-8-safe preview in context while the full payload is
written privately under `artifacts/<session>/mcp/` for main, ACP, and subagent
sessions; JSON and long-line results include query guidance.
