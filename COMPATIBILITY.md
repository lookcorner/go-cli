...
| Shell execution | partial | Gork-compatible tools support foreground exit status, background timeout/lifecycle, multi-task wait/poll, process groups, cleanup, and replayed cwd/environment/functions/aliases; ACP interactive PTY create/input/load/resize/list/kill with bounded replay also works on Unix; shell-option parity, PTY activity-transition notifications, Windows ConPTY and cgroups remain |
