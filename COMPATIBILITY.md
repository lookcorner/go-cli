...
| Git / worktrees / hunk tracking | partial | Git tracked/staged/untracked hunk parsing, stable IDs, file summaries, file-tool agent attribution, ACP read queries and safe hunk/file/all actions work; persisted linked/standalone/git lifecycle, sync/async fork creation, dirty-state copying, status notifications, overwrite/merge conflict reporting, safe removal, GC and registry stats/rebuild/path management work; mixed-source per-hunk attribution, prompt-index turn actions, session resume/rehydrate, snapshot disposal and optimized CoW copying remain |
