...
| Skills / AGENTS.md | partial | Compatible home-level and Git-root-to-CWD scoped instruction/rules and skill discovery, deeper-scope precedence, `GROK_HOME`, source-labelled injection, Git-native instruction ignore filtering, lazy `skill` loading and `paths`-gated activation after file tools work; vendor compatibility gates and dynamic skill rediscovery remain |
