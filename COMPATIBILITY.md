...
| Session persistence | partial | Durable JSONL events, latest/path resume, completed-turn response-ID continuation, checkpoint-aligned transcript reconstruction, append-only conversation branches and rewind points/execute support `all`, `conversation_only`, `files_only`/`code_only`; file-tool before/after checkpoints are durable, conflict-aware and force-restorable across Responses, Chat Completions and Anthropic histories; shell-created file changes, general session fork and migrations remain |
