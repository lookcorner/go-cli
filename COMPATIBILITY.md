...
| Git / worktrees / hunk tracking | partial | Git tracked/staged/untracked hunk parsing, stable IDs, file summaries, file-tool agent attribution, ACP read queries and safe hunk/file/all actions work; persisted linked/standalone/git create/list/show/remove lifecycle, dirty-state copying, status notifications and safe dry-run removal work; mixed-source per-hunk attribution, prompt-index turn actions, apply/fork/resume/gc/database-management extensions and optimized CoW copying remain |
