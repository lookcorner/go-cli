...
func TestNamedSessionMetadataAndList(t *testing.T) {
	...
	if err := logger.Append("session_metadata", map[string]any{"cwd": "/workspace/project", "worktreeLabel": "feature-one", "headCommit": "abc123", "headBranch": "main"}); err != nil {
		t.Fatal(err)
	}
	...
	if len(items) != 1 || items[0].SessionID != "acp-session-1" || items[0].Title != "Implement the persistent session support" || items[0].WorktreeLabel != "feature-one" || items[0].HeadCommit != "abc123" || items[0].HeadBranch != "main" || items[0].CreatedAt.IsZero() || items[0].CreatedAt.After(items[0].UpdatedAt) {
