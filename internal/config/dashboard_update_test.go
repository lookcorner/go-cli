...
func TestDashboardPinnedConfigRoundTrip(t *testing.T) {
	...
	cfg, err := Load(path)
	if err != nil || !reflect.DeepEqual(cfg.Dashboard.Pinned, []string{"a", "z"}) || !reflect.DeepEqual(cfg.Dashboard.Reorder, []string{"z", "a"}) || cfg.Dashboard.Grouping != "directory" {
	...
	cfg, err = Load(path)
	if err != nil || !reflect.DeepEqual(cfg.Dashboard.Pinned, []string{"a", "b"}) || !reflect.DeepEqual(cfg.Dashboard.Reorder, []string{"z", "a"}) || cfg.Dashboard.Grouping != "directory" {
	...
	cfg, err = Load(path)
	if err != nil || !reflect.DeepEqual(cfg.Dashboard.Pinned, []string{"a", "b"}) || !reflect.DeepEqual(cfg.Dashboard.Reorder, []string{"b", "a"}) || cfg.Dashboard.Grouping != "directory" {
	...
	cfg, err = Load(path)
	if err != nil || cfg.Dashboard.Grouping != "state" || !reflect.DeepEqual(cfg.Dashboard.Pinned, []string{"a", "b"}) || !reflect.DeepEqual(cfg.Dashboard.Reorder, []string{"b", "a"}) {
...
func TestDashboardGroupingDefaultsToState(t *testing.T) {
	...
	for _, content := range []string{"", "[dashboard]\ngrouping = \"garbage\"\n"} {
		...
		cfg, err := Load(path)
		if err != nil || cfg.Dashboard.Grouping != "state" {
			t.Fatalf("content=%q grouping=%q err=%v", content, cfg.Dashboard.Grouping, err)
		}
	}
}
