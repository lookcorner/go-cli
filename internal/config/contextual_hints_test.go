...
func TestContextualHintsDefaultAndLocalConfig(t *testing.T) {
	...
	if err != nil || !cfg.UI.ContextualHints.Undo || !cfg.UI.ContextualHints.SendNow {
	...
	for _, test := range []struct {
		...
		{name: "json", file: "config.json", content: `{"ui":{"contextual_hints":{"undo":false,"send_now":false}}}`},
	} {
	...
	for _, enabled := range []bool{false, true} {
		t.Run(map[bool]string{false: "off", true: "on"}[enabled], func(t *testing.T) {
			...
			cfg.ApplyRemoteSettings(&RemoteSettings{ContextualHints: &RemoteHints{
				Undo: boolPointer(!enabled), SendNow: boolPointer(!enabled),
			}})
			if cfg.UI.ContextualHints.Undo != enabled || cfg.UI.ContextualHints.SendNow != enabled {
...
func TestContextualHintsEnvironmentOverridesRequirements(t *testing.T) {
	...
	if cfg.UI.ContextualHints.Undo || cfg.UI.ContextualHints.SendNow {
...
func TestUpdateContextualUndoHintPreservesConfig(t *testing.T) {
	for _, test := range []struct {
		...
		t.Run(test.name, func(t *testing.T) {
			...
			if root["model_name"] != "test" || hints["plan_mode"] != false || hints["undo"] != false || hints["send_now"] != false {
...
func TestRemoteSettingsDecodesContextualHints(t *testing.T) {
	...
	if err := json.Unmarshal([]byte(`{"contextual_hints":{"undo":false,"send_now":true}}`), &settings); err != nil {
		t.Fatal(err)
	}
	if settings.ContextualHints == nil || settings.ContextualHints.Undo == nil || *settings.ContextualHints.Undo || settings.ContextualHints.SendNow == nil || !*settings.ContextualHints.SendNow {
