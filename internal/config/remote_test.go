...
func TestApplyRemoteSettingsUsesLocalAndEnvironmentPrecedence(t *testing.T) {
	...
	cfg.ApplyRemoteSettings(&RemoteSettings{
		OfficialMarketplaceAutoRegister: boolPointer(true), WebFetchEnabled: boolPointer(true),
	...
	if cfg.OfficialMarketplaceAutoRegister || cfg.WebFetch.Enabled || cfg.WebFetch.ProxyEndpoint != "https://local.example" || !cfg.Compat.Cursor.Skills || cfg.Compat.Claude.Hooks || !cfg.WebFetch.DomainsConfigured || len(cfg.WebFetch.AllowedDomains) != 1 {
