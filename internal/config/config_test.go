...
func TestLoadAndValidateBashConfig(t *testing.T) {
	...
	if cfg.Toolset.Bash.TimeoutSeconds != 120 || cfg.Toolset.Bash.MaxTimeoutSeconds != 36000 || cfg.Toolset.Bash.OutputByteLimit != 20000 || !cfg.Toolset.Bash.AllowBackgroundOperator || !cfg.Toolset.Bash.AutoBackgroundOnTimeout || cfg.Toolset.Bash.ForegroundBlockBudgetMS != nil {
		t.Fatalf("default bash config=%#v", cfg.Toolset.Bash)
	}
	if err := os.WriteFile(path, []byte("[toolset.bash]\ntimeout_secs = 0.05\nmax_timeout_secs = 30\noutput_byte_limit = 64\ncmd_prefix = 'source ./env.sh'\nallow_background_operator = false\nauto_background_on_timeout = false\nforeground_block_budget_ms = 25\n"), 0o600); err != nil {
		t.Fatal(err)
	}
	cfg, err = Load(path)
	if err != nil || cfg.Toolset.Bash.TimeoutSeconds != 0.05 || cfg.Toolset.Bash.MaxTimeoutSeconds != 30 || cfg.Toolset.Bash.OutputByteLimit != 64 || cfg.Toolset.Bash.CommandPrefix != "source ./env.sh" || cfg.Toolset.Bash.AllowBackgroundOperator || cfg.Toolset.Bash.AutoBackgroundOnTimeout || cfg.Toolset.Bash.ForegroundBlockBudgetMS == nil || *cfg.Toolset.Bash.ForegroundBlockBudgetMS != 25 {
...
func TestThemeConfigCanonicalizationAndUpdate(t *testing.T) {
	...
	if _, err := Load(invalidPath); err == nil || !strings.Contains(err.Error(), "ui screen_mode") {
		t.Fatalf("invalid screen mode error=%v", err)
	}
}
...
func TestScrollEnvironmentOverridesConfig(t *testing.T) {
	t.Setenv("GROK_SCROLL_SPEED", "120")
	...
	if cfg.UI.ScrollSpeed != 100 || cfg.UI.ScrollLines == nil || *cfg.UI.ScrollLines != 10 || !cfg.UI.InvertScroll {
		t.Fatalf("environment override=%#v", cfg.UI)
	}
	t.Setenv("GROK_SCROLL_SPEED", "fast")
	...
	if cfg.UI.ScrollSpeed != 25 || cfg.UI.ScrollLines == nil || *cfg.UI.ScrollLines != 4 || cfg.UI.InvertScroll {
		t.Fatalf("invalid environment value did not preserve config=%#v", cfg.UI)
	}
	t.Setenv("GROK_SCROLL_LINES", "0")
	t.Setenv("GROK_SCROLL_SPEED", "0")
	cfg, err = Load(path)
	if err != nil {
		t.Fatal(err)
	}
	if cfg.UI.ScrollSpeed != 1 || cfg.UI.ScrollLines != nil {
		t.Fatalf("zero environment values were not normalized=%#v", cfg.UI)
	}
}
...
func TestUIConfigPersistenceAndPermissionPrecedence(t *testing.T) {
	...
	if err := UpdateDefaultSelectedPermission(path, "reject"); err != nil {
		t.Fatal(err)
	}
	if err := UpdateQuestionTimeout(path, false); err != nil {
		t.Fatal(err)
	}
	if err := UpdateInvertScroll(path, true); err != nil {
		t.Fatal(err)
	}
	if err := UpdateScrollSpeed(path, 75); err != nil {
		t.Fatal(err)
	}
	if err := UpdateScrollMode(path, "wheel"); err != nil {
		t.Fatal(err)
	}
	if err := UpdateScrollLines(path, 5); err != nil {
		t.Fatal(err)
	}
	if err := UpdateTextSelection(path, "hold"); err != nil {
		t.Fatal(err)
	}
	cfg, err := Load(path)
	if err != nil {
	...
	if err := UpdateDefaultSelectedPermission(emptyPath, "allow_once"); err != nil {
		t.Fatal(err)
	}
	if err := UpdateQuestionTimeout(emptyPath, false); err != nil {
		t.Fatal(err)
	}
	if err := UpdateInvertScroll(emptyPath, true); err != nil {
		t.Fatal(err)
	}
	if err := UpdateScrollSpeed(emptyPath, 75); err != nil {
		t.Fatal(err)
	}
	if err := UpdateScrollMode(emptyPath, "wheel"); err != nil {
		t.Fatal(err)
	}
	if err := UpdateScrollLines(emptyPath, 5); err != nil {
		t.Fatal(err)
	}
	if err := UpdateTextSelection(emptyPath, "hold"); err != nil {
		t.Fatal(err)
	}
	...
	if err := UpdateDefaultSelectedPermission(jsonPath, "allow_command_always"); err != nil {
		t.Fatal(err)
	}
	if err := UpdateQuestionTimeout(jsonPath, false); err != nil {
		t.Fatal(err)
	}
	if err := UpdateInvertScroll(jsonPath, true); err != nil {
		t.Fatal(err)
	}
	if err := UpdateScrollSpeed(jsonPath, 75); err != nil {
		t.Fatal(err)
	}
	if err := UpdateScrollMode(jsonPath, "trackpad"); err != nil {
		t.Fatal(err)
	}
	if err := UpdateScrollLines(jsonPath, 5); err != nil {
		t.Fatal(err)
	}
	if err := UpdateTextSelection(jsonPath, "word_select"); err != nil {
		t.Fatal(err)
	}
	...
	if err := UpdateDefaultSelectedPermission(jsonPath, "invalid"); err == nil {
		t.Fatal("invalid default selected permission was accepted")
	}
	if err := UpdateDefaultSelectedPermission(jsonPath, " REJECT "); err == nil {
		t.Fatal("non-canonical default selected permission was accepted")
	}
	if err := UpdateTextSelection(jsonPath, "invalid"); err == nil {
		t.Fatal("invalid text selection mode was accepted")
	}
	if err := UpdateScrollMode(jsonPath, "invalid"); err == nil {
		t.Fatal("invalid scroll mode was accepted")
	}
	if err := UpdateScrollSpeed(jsonPath, 0); err != nil {
		t.Fatal(err)
	}
	if err := UpdateScrollLines(jsonPath, 20); err != nil {
		t.Fatal(err)
	}
	clampedConfig, err := Load(jsonPath)
	if err != nil || clampedConfig.UI.ScrollSpeed != 1 || clampedConfig.UI.ScrollLines == nil || *clampedConfig.UI.ScrollLines != 10 {
		t.Fatalf("clamped config=%#v err=%v", clampedConfig.UI, err)
	}
