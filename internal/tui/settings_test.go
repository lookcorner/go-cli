...
func TestSettingsPanelPersistsEverySupportedSetting(t *testing.T) {
	...
	m := &model{
		...
		persistPageFlip:   func(value bool) error { booleans = append(booleans, "page-flip"); return nil },
		...
		persistRefresh: func(value bool) error {
			booleans = append(booleans, "display-refresh")
			return nil
		},
		persistShowTips: func(value bool) error {
			booleans = append(booleans, "show-tips")
			return nil
		},
		persistManualFolds: func(value bool) error {
			booleans = append(booleans, "manual-folds")
			return nil
		},
		...
		wordSelectHint: wordSelectHintState{
			persist: func(value bool) error {
				booleans = append(booleans, "word-select-hint")
				return nil
			},
		},
		sshWrapHint: smallScreenHintState{
			persist: func(value bool) error {
				booleans = append(booleans, "ssh-wrap-hint")
				return nil
			},
		},
	...
	for index := 0; index < settingsCount; index++ {
		...
		if index == 8 || index == 9 || index == 17 || index == 34 || index == 38 {
...
func TestSettingsDisplayRefreshRollsBackPersistenceFailure(t *testing.T) {
	m := &model{
		matchRefresh: true,
		persistRefresh: func(bool) error {
			return errors.New("read only")
		},
		settings: &settingsState{selected: 38},
	}
	updated, command := m.Update(tea.KeyPressMsg(tea.Key{Code: tea.KeyEnter}))
	m = updated.(*model)
...
func TestSettingsSSHWrapHintRollsBackPersistenceFailure(t *testing.T) {
	m := &model{
		sshWrapHint: smallScreenHintState{
			enabled: true, active: true,
			persist: func(bool) error { return errors.New("read only") },
		},
		settings: &settingsState{selected: 37},
	}
	updated, command := m.Update(tea.KeyPressMsg(tea.Key{Code: tea.KeyEnter}))
	m = updated.(*model)
	if command != nil || !m.sshWrapHint.enabled || !m.sshWrapHint.active ||
		m.settings.err != "read only" || m.status != "setting update failed" {
		t.Fatalf("command=%v hint=%#v err=%q status=%q", command != nil, m.sshWrapHint, m.settings.err, m.status)
	}
}

func TestSettingsSSHWrapHintDisableRetiresActiveHint(t *testing.T) {
	m := &model{
		sshWrapHint: smallScreenHintState{enabled: true, active: true},
		settings:    &settingsState{selected: 37},
	}
	updated, command := m.Update(tea.KeyPressMsg(tea.Key{Code: tea.KeyEnter}))
	m = updated.(*model)
	if command != nil || m.sshWrapHint.enabled || m.sshWrapHint.active || m.status != "settings updated" {
		t.Fatalf("command=%v hint=%#v status=%q", command != nil, m.sshWrapHint, m.status)
	}
}
...
func TestSettingsPageFlipRollsBackPersistenceFailure(t *testing.T) {
	m := &model{
		pageFlipOnSend: true,
		persistPageFlip: func(bool) error {
			return errors.New("read only")
		},
		settings: &settingsState{selected: 35},
	}
	updated, command := m.Update(tea.KeyPressMsg(tea.Key{Code: tea.KeyEnter}))
	m = updated.(*model)
	if command != nil || !m.pageFlipOnSend || m.settings.err != "read only" || m.status != "setting update failed" {
		t.Fatalf("command=%v enabled=%v err=%q status=%q", command != nil, m.pageFlipOnSend, m.settings.err, m.status)
	}
}

func TestSettingsShowTipsRollsBackPersistenceFailure(t *testing.T) {
	m := &model{
		showTips: true,
		persistShowTips: func(bool) error {
			return errors.New("read only")
		},
		settings: &settingsState{selected: 34},
	}
	updated, command := m.Update(tea.KeyPressMsg(tea.Key{Code: tea.KeyEnter}))
	m = updated.(*model)
	if command != nil || !m.showTips || m.settings.err != "read only" || m.status != "setting update failed" {
		t.Fatalf("command=%v enabled=%v err=%q status=%q", command != nil, m.showTips, m.settings.err, m.status)
	}
}

func TestSettingsRespectManualFoldsRollsBackPersistenceFailure(t *testing.T) {
	m := &model{
		respectManualFolds: true,
	...
	updated, command := m.Update(tea.KeyPressMsg(tea.Key{Code: tea.KeyEnter}))
	m = updated.(*model)
	if command != nil || !m.respectManualFolds || m.settings.err != "read only" || m.status != "setting update failed" {
		t.Fatalf("command=%v enabled=%v err=%q status=%q", command != nil, m.respectManualFolds, m.settings.err, m.status)
	}
}

func TestSettingsHideDisplayRefreshInMinimalMode(t *testing.T) {
	m := &model{minimal: true, settings: &settingsState{}}
	if m.settingsCount() != settingsCount-1 || strings.Contains(m.settingsContent(), "Match display refresh rate") {
...
func TestSettingsContextualWordSelectHintRollsBackPersistenceFailure(t *testing.T) {
	m := &model{
		wordSelectHint: wordSelectHintState{
			enabled: true,
			persist: func(bool) error {
	...
	updated, command := m.Update(tea.KeyPressMsg(tea.Key{Code: tea.KeyEnter}))
	m = updated.(*model)
	if command != nil || !m.wordSelectHint.enabled || m.settings.err != "read only" || m.status != "setting update failed" {
		t.Fatalf("command=%v enabled=%v err=%q status=%q", command != nil, m.wordSelectHint.enabled, m.settings.err, m.status)
	}
}
