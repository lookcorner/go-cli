...
// kittyRowDiacritics maps a placeholder row index to its combining character,
// following rowcolumn-diacritics.txt from the kitty graphics protocol.
var kittyRowDiacritics = [...]rune{
	...
	0x082D, 0x0951, 0x0953, 0x0954, 0x0F82, 0x0F83, 0x0F86, 0x0F87, 0x135D, 0x135E, 0x135F, 0x17DD, 0x193A, 0x1A17, 0x1A75, 0x1A76,
	...
	0x2DFB, 0x2DFC, 0x2DFD, 0x2DFE, 0x2DFF, 0xA66F, 0xA67C, 0xA67D, 0xA6F0, 0xA6F1, 0xA8E0, 0xA8E1, 0xA8E2, 0xA8E3, 0xA8E4, 0xA8E5,
	0xA8E6, 0xA8E7, 0xA8E8, 0xA8E9, 0xA8EA, 0xA8EB, 0xA8EC, 0xA8ED, 0xA8EE, 0xA8EF, 0xA8F0, 0xA8F1, 0xAAB0, 0xAAB2, 0xAAB3, 0xAAB7,
	0xAAB8, 0xAABE, 0xAABF, 0xAAC1, 0xFE20, 0xFE21, 0xFE22, 0xFE23, 0xFE24, 0xFE25, 0xFE26, 0x10A0F, 0x10A38, 0x1D185, 0x1D186, 0x1D187,
	0x1D188, 0x1D189, 0x1D1AA, 0x1D1AB, 0x1D1AC, 0x1D1AD, 0x1D242, 0x1D243, 0x1D244}

const kittyPlaceholder = "\U0010EEEE"

// kittyPlaceholderGrid renders the Unicode placeholder grid for a virtual
// kitty image placement. Only the first cell of each row carries a diacritic;
// the remaining cells inherit their column from it.
func kittyPlaceholderGrid(cols, rows int) string {
	lines := make([]string, 0, rows)
	for row := 0; row < rows; row++ {
		line := kittyPlaceholder
		if row < len(kittyRowDiacritics) {
			line += string(rune(kittyRowDiacritics[row]))
		}
		if cols > 1 {
			line += strings.Repeat(kittyPlaceholder, cols-1)
		}
...
// kittyTransmitVirtual transmits image data and creates a virtual placement
// for Unicode placeholders in one command, suppressing terminal responses.
func kittyTransmitVirtual(id int, data []byte, cols, rows int) []byte {
	...
	for len(encoded) > 0 {
		...
		if first {
			fmt.Fprintf(&out, "\x1b_Ga=T,f=100,q=2,i=%d,U=1,c=%d,r=%d,m=%d;", id, cols, rows, more)
			first = false
		} else {
...
// kittyPlaceholderColor returns the true-color SGR foreground encoding a
// 24-bit kitty image ID for placeholder cells.
func kittyPlaceholderColor(id int) string {
	return fmt.Sprintf("\x1b[38;2;%d;%d;%dm", id>>16&0xff, id>>8&0xff, id&0xff)
}
