...
const SlashUsage = "Usage: /doctor [fix [ssh-wrap|tmux-clipboard|dcs-passthrough|tmux-extended-keys|tmux-truecolor|colorterm] [--yes]]"
...
func SuggestFixArgs(argsQuery string) []string {
	...
	if query == "fix" || strings.HasPrefix(query, "fix ") {
		...
		var out []string
		for _, handle := range []string{SSHWrapHandle, TmuxClipboardHandle, DCSPassthroughHandle, TmuxExtendedKeysHandle, TmuxTruecolorHandle, ColortermHandle} {
