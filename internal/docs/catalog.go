...
var guides = []Guide{
	...
	guide("Theming and Appearance", "Themes, timestamps, timeline, and compact display", "Use `/settings` for the supported display preferences or `/theme`, `/timestamps`, `/timeline`, `/compact-mode`, and `/vim-mode` directly. Closed Mermaid flowchart, state, sequence, class, and ER fences render as Unicode art; unsupported or incomplete diagrams keep their source. Set `[ui].render_mermaid` to `auto`, `on`, or `off`. Successful changes are written atomically."),
	...
	guide("Custom Models", "BYOK and compatible model endpoints", "Define model entries with provider, endpoint, context window, and reasoning metadata. `gork models` lists the resolved default and visible catalog. `/model` switches an idle session while preserving completed text and image history when compatible."),
	...
	guide("Session Management", "Save, load, resume, fork, rewind, and compact", "Sessions are stored as local JSONL. Use `/resume` to switch, `/fork` to branch completed history, `/rewind` to restore a turn checkpoint, `/compact` to summarize, and `/new` to start fresh. `gork sessions list|search|delete` manages history outside the TUI, and `gork export <session-id>` writes completed history as Markdown."),
	guide("Sandbox Mode", "Shell process filesystem confinement", "Use `--sandbox workspace` to limit model-started shell writes to the workspace, temporary directories, and `~/.grok`; `--sandbox read-only` removes workspace writes. On Linux, `--sandbox strict` also limits reads to the workspace and required runtime paths, and read-only/strict child processes have no network. macOS uses Seatbelt and Linux uses bubblewrap; strict currently requires Linux. File tools retain their workspace and symlink checks."),
	...
	guide("Terminal Support and Troubleshooting", "tmux, SSH, color, clipboard, and mouse diagnostics", "Run `gork doctor` or `/doctor` (`/terminal-setup`) to inspect color, clipboard, tmux, SSH, and mouse-reporting behavior. Use `gork doctor fix` or `/doctor fix` for managed SSH-wrap and tmux option setup. The TUI supports OSC 52 clipboard transfer and tmux-aware OSC 8 links."),
