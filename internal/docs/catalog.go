...
var guides = []Guide{
	...
	guide("Session Management", "Save, load, resume, fork, rewind, and compact", "Sessions are stored as local JSONL. Use `/resume` to switch, `/fork` to branch completed history, `/rewind` to restore a turn checkpoint, `/compact` to summarize, and `/new` to start fresh. `gork sessions list|search|delete` manages history outside the TUI."),
