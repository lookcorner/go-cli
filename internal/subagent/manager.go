...
type persistedTask struct {
	...
	Bypass         bool       `json:"bypass_permissions,omitempty"`
}
...
func New(config Config) (*Manager, error) {
	...
	manager := &Manager{
		...
		autoWake: config.AutoWake, cancelWake: config.CancelWake, disablePermissionBypass: config.DisablePermissionBypass, tasks: make(map[string]*task),
	}
...
func (m *Manager) Start(ctx context.Context, request tools.SubagentRequest) (tools.SubagentResult, error) {
	...
	bypass := runtimeDefinition.PermissionMode == "bypassPermissions" && runtimeDefinition.Plugin == "" && !m.disablePermissionBypass
...
func (m *Manager) launch(caller context.Context, current *task, prompt string, background bool) (tools.SubagentResult, error) {
	...
	if current.bypass {
		runCtx = tools.WithPermissionBypass(runCtx)
	}
	current.mu.Lock()
...
func (m *Manager) startPersistence(current *task, prompt, resumedFrom string) error {
	...
	if err := writeTaskMeta(current.metaPath, persistedTask{
		...
		HarnessType: current.harnessType, CapabilityMode: current.capability, Bypass: current.bypass,
	}); err != nil {
		_ = logger.Close()
		_ = os.Remove(logger.Path())
		return err
...
func (m *Manager) loadPersistedTasks() []tools.SubagentResult {
	...
	for _, entry := range entries {
		...
		current := &task{
			...
			resumedFrom: record.ResumedFrom, metaPath: path, terminal: true, bypass: record.Bypass && !m.disablePermissionBypass,
		}
		close(current.done)
